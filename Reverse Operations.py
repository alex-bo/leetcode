...
def reverse(head):
    ...
    fake_head = Node(0)
    fake_head.next = head
    prev, cur = fake_head, fake_head.next
    # printLinkedList(fake_head)
    while cur:
        prev.next = push_down(cur)
        if prev.next != cur:  # cur node was pushed down
            # print()
            # printLinkedList(fake_head)
            cur = prev.next
        else:
...
def push_down(node: Node) -> Node:
    fake_head = Node(0)
    fake_head.next = node
    prev, cur = fake_head, fake_head.next
    while cur and cur.next and cur.data % 2 == 0 and cur.next.data % 2 == 0 and cur.data < cur.next.data:
        prev.next = cur.next
